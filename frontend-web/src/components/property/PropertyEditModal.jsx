...
export default function PropertyEditModal({ property, onClose, onSubmit, isSaving }) {
  const [form, setForm] = useState({ name: '', address: '', city: '', image: null });
  const [error, setError] = useState('');

  useEffect(() => {
    if (property) setForm({ name: property.name, address: property.address, city: property.city, image: null });
    setError('');
  }, [property]);
  ...
  const submit = async (event) => {
    ...
    if (form.image && (!form.image.type.startsWith('image/') || form.image.size > 10 * 1024 * 1024)) { setError('Choose an image up to 10 MB.'); return; }
    try { await onSubmit({ name: form.name.trim(), address: form.address.trim(), city: form.city.trim(), image: form.image }); onClose(); }
  ...
  return <ModalShell title="Edit Property" subtitle="Update the property name and location" onClose={onClose}>
    <form onSubmit={submit} className="p-6 space-y-4">
      ...
      <Field label="Replace Property Image (optional)"><input type="file" accept="image/*" onChange={(e) => setForm({ ...form, image: e.target.files?.[0] || null })} className={INPUT_CLASS} /></Field>
