...
function PropertiesPanel({ properties, onView, onEdit, onDelete }) {
  return <section className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">{!properties.length ? <EmptyState icon={Building2} title="No properties yet" message="Add your first property to begin managing units." /> : <div className="overflow-x-auto"><table className="w-full text-left"><thead className="bg-gray-50/80 border-b border-gray-100"><tr>{['Property', 'Address', 'Units', 'Actions'].map((heading) => <th key={heading} className="px-5 py-3 text-xs font-bold uppercase tracking-wider text-gray-500">{heading}</th>)}</tr></thead><tbody className="divide-y divide-gray-100">{properties.map((property) => <tr key={property.id} className="hover:bg-gray-50/60"><td className="px-5 py-4"><div className="flex items-center gap-3">{property.imageUrl ? <img src={property.imageUrl} alt="" className="w-12 h-12 rounded-lg object-cover bg-gray-100" /> : <div className="w-12 h-12 rounded-lg bg-gray-100 flex items-center justify-center"><Building2 className="w-5 h-5 text-gray-400" /></div>}<span className="font-semibold text-sm text-gray-900">{property.name}</span></div></td><td className="px-5 py-4 text-sm text-gray-600">{property.address}, {property.city}</td><td className="px-5 py-4 text-sm text-gray-600">{property.units?.length || 0}</td><td className="px-5 py-4"><div className="flex gap-1"><button title="View property" onClick={() => onView(property)} className="p-2 text-gray-600 hover:bg-gray-100 rounded-lg"><Eye className="w-4 h-4" /></button><RowActions onEdit={() => onEdit(property)} onDelete={() => onDelete(property)} /></div></td></tr>)}</tbody></table></div>}</section>;
}

function PropertyDetails({ property, onClose }) { return <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40"><div className="bg-white rounded-xl shadow-xl w-full max-w-xl overflow-hidden">{property.imageUrl && <img src={property.imageUrl} alt={property.name} className="w-full h-52 object-cover" />}<div className="p-6 border-b border-gray-100 flex justify-between"><div><h2 className="text-xl font-bold text-gray-900">{property.name}</h2><p className="text-sm text-gray-500 mt-1">{property.address}, {property.city}</p></div><button onClick={onClose} className="text-gray-500">Close</button></div><div className="p-6"><h3 className="text-xs font-bold uppercase tracking-wider text-gray-500 mb-3">Registered Units</h3><div className="space-y-2">{property.units?.length ? property.units.map((unit) => <div key={unit.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg"><span className="text-sm font-semibold text-gray-800">Unit {unit.unitNumber} - Floor {unit.floor}</span><Status status={unit.status} /></div>) : <p className="text-sm text-gray-500">No units registered.</p>}</div></div></div></div>; }
